...
const Projects = () => {
  const projects = [
    {
      title: "BUYHAC - SaaS Price Tracker & Alert System",
      date: "Mar 2026",
      featured: true,
      overview:
        "Developed a high-performance SaaS application that tracks product prices across multiple e-commerce platforms. The system provides real-time price history, automated tracking, and email notifications when products reach a target price using web scraping and cloud-based automation.",
      keyFeatures: [
        "Automated Price Tracking - Users can paste any product URL, and the system extracts product details like name, image, and current price automatically.",
        "Dynamic Web Scraping Engine - Integrated Firecrawl to handle JavaScript-heavy websites and bypass anti-bot restrictions for accurate data extraction.",
        "Price History Visualization - Implemented interactive charts to display price trends and fluctuations over time.",
        "Automated CRON Jobs - Scheduled background jobs to perform daily price checks, ensuring up-to-date data without manual intervention.",
        "Email Notifications System - Configured Resend to send alerts when a product reaches the user’s target price.",
        "Secure Authentication - Implemented Google OAuth with Supabase Auth for secure login and personalized dashboards.",
        "Server Actions Integration - Used Next.js Server Actions for type-safe and secure communication between frontend and backend.",
      ],
      techStack: [
        "Next.js",
        "React",
      ...
      image: "",
      liveUrl: "https://buyhac.vercel.app/",
  ...
  return (
    <section
      id="projects"
      className="py-24 px-6 bg-gradient-to-b from-gray-950 to-black text-white relative overflow-hidden"
    >
      {/* Background elements */}
        ...
        >
          {projects.map((project, index) => (
            ...
            >
              ...
              <div
                className={`absolute inset-0 rounded-3xl bg-gradient-to-r ${project.gradient} opacity-0 group-hover:opacity-5 transition-opacity duration-500 pointer-events-none`}
              ></div>
            </motion.div>
          ))}
        </motion.div>
